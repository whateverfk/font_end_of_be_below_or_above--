  ...
  <div class="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 pb-12">
    ...
    <div class="lg:col-span-1 space-y-6">
      ...
      <div class="glass-card p-6 overflow-hidden relative group">
        ...
        <div class="space-y-6 relative z-10">
          ...
          <div :class="{ 'opacity-50 pointer-events-none': !syncStore.isAutoSyncEnabled }" class="transition-opacity">
            <label class="block text-xs font-bold text-zinc-500 uppercase tracking-widest mb-2 ml-1">Interval (Minutes)</label>
            <div class="relative">
              <input 
      ...
      <div class="glass-card p-6 bg-gradient-to-br from-white/[0.03] to-teal-500/[0.02]">
        ...
        <p class="text-sm text-zinc-500 mb-6">Instantly trigger a synchronization cycle across all connected devices.</p>
    ...
    <div class="lg:col-span-2 glass-card flex flex-col min-h-[500px] border-white/5">
      <div class="p-6 border-b border-white/5 bg-white/[0.02] flex items-center justify-between">
        ...
        <span class="px-2 py-1 rounded-md bg-zinc-900 border border-white/10 text-[10px] text-zinc-500 font-black uppercase tracking-widest">Live Logs</span>
      </div>
      
      <div class="flex-1 p-6 overflow-y-auto custom-scrollbar bg-zinc-950/20">
        <div v-if="syncStore.syncLogs.length === 0" class="h-full flex flex-col items-center justify-center text-zinc-600 space-y-3">
        ...
        <div v-else class="space-y-4">
          <TransitionGroup name="log">
            ...
            >
              <div class="mt-1">
                <CheckCircle2 v-if="log.status === 'Success'" class="w-4 h-4 text-emerald-500" />
                <AlertCircle v-else class="w-4 h-4 text-rose-500" />
              </div>
              <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between mb-1">
                  <span class="text-[10px] text-zinc-600 uppercase font-black tracking-widest">{{ log.time }}</span>
                  <span class="text-[10px] px-1.5 py-0.5 rounded" :class="log.status === 'Success' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'">
                    {{ log.status }}
                  </span>
                </div>
                <p class="text-zinc-300 text-sm break-words">{{ log.message }}</p>
              </div>
            </div>
          </TransitionGroup>
        </div>
...
import { 
  Settings2, RefreshCw, Clock, Zap, 
  History, CheckCircle2, AlertCircle, Terminal 
...
.log-enter-active { transition: all 0.4s ease; }
