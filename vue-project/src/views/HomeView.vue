  ...
  <div class="max-w-7xl mx-auto space-y-10 pb-20">
    <div class="flex items-center justify-between">
      <h2 class="text-3xl font-black text-zinc-100 flex items-center gap-4">
        <div
          class="w-12 h-12 rounded-2xl bg-teal-500/10 flex items-center justify-center text-teal-400 border border-teal-500/20"
        >
    ...
    <div class="grid grid-cols-1 xl:grid-cols-4 gap-8">
      ...
      <div class="xl:col-span-1 space-y-6">
        <div
          class="glass-card p-6 flex items-center gap-4 border-white/5 relative overflow-hidden group"
        >
          <div
            class="absolute -right-4 -bottom-4 opacity-5 group-hover:scale-110 transition-transform"
          >
            <Monitor class="w-24 h-24 text-teal-500" />
          </div>
          <div
            class="w-14 h-14 rounded-2xl bg-teal-500/10 flex items-center justify-center text-teal-400 border border-teal-500/10"
          >
            <Monitor class="w-7 h-7" />
          </div>
          <div>
        ...
        >
          <div
            class="absolute -right-4 -bottom-4 opacity-5 group-hover:scale-110 transition-transform text-emerald-500"
          >
            <div class="w-24 h-24 rounded-full border-8 border-current opacity-10"></div>
          </div>
          <div
            class="w-14 h-14 rounded-2xl bg-emerald-500/10 flex items-center justify-center text-emerald-400 border border-emerald-500/10"
          >
        ...
        <div class="glass-card p-8 border-white/5 bg-gradient-to-br from-teal-500/5 to-transparent">
          ...
          <p class="text-sm text-zinc-500 leading-relaxed">
            Bấm vào device Ip để xem thông tin chi tiết, live view, device log ...
          </p>
        </div>
      </div>
            ...
            <tbody class="divide-y divide-white/5">
              ...
              >
                <td class="px-8 py-5">
                  <router-link
                    :to="`/device/${device.id}`"
                    class="flex flex-col hover:text-teal-400 transition-colors"
                  >
                    <span class="font-mono font-bold text-zinc-200">{{ device.ipNvr }}</span>
                    <span class="text-[10px] text-zinc-500 uppercase font-black tracking-tighter">{{
                      device.brand
                    }}</span>
                  </router-link>
                </td>
                ...
                <td class="px-8 py-5">
                  ...
                  >
                    <div
                      ...
                      :class="[
                        device.status
                          ? 'translate-x-5 bg-emerald-400 shadow-emerald-500/50'
    ...
    <Transition name="modal">
      <div v-if="showModal" class="fixed inset-0 z-[100] flex items-center justify-center p-6">
        ...
        <div
          class="glass-card w-full max-w-lg p-10 relative z-10 border-teal-500/20 shadow-4xl animate-in zoom-in-95 duration-200"
        >
          <div class="flex items-center justify-between mb-8">
            <div>
              <h3 class="text-2xl font-black text-zinc-100 tracking-tight">
                {{ isEditMode ? 'Modify Infrastructure' : 'Register New Device' }}
              </h3>
              <p class="text-zinc-500 text-sm mt-1">
                {{
                  isEditMode
                    ? 'Update existing hardware parameters and credentials.'
                    : 'Integrate a new camera system into your network cluster.'
                }}
              </p>
            </div>
            <button @click="closeModal" class="text-zinc-600 hover:text-rose-400 transition-colors">
              <X class="w-8 h-8" />
            </button>
          </div>
          ...
          <form @submit.prevent="handleSubmit" class="grid grid-cols-2 gap-6">
            ...
            <div class="col-span-2">
              <label
                class="block text-[10px] font-black uppercase text-zinc-600 tracking-widest mb-2 ml-1"
                >Hardware Brand Entity</label
              >
              <select
                v-model="form.brand"
                class="w-full bg-zinc-950 border border-white/10 rounded-2xl py-3.5 px-5 text-sm focus:outline-none focus:border-teal-500/50 appearance-none text-zinc-300"
              >
            ...
            <div class="col-span-2 flex gap-4 mt-6">
              <button
                type="button"
                @click="closeModal"
                class="flex-1 py-4 bg-zinc-900 text-zinc-400 font-bold rounded-2xl hover:bg-zinc-800 transition-colors"
              >
                Cancel
              </button>
              <button
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const testResult = await deviceStore.testConnection(form)

    if (!testResult.ip_reachable) {
      notify('Network Unreachable', testResult.message || 'Cannot reach device IP', 'error')
    ...
    if (!testResult.auth_ok) {
      notify(
        ...
        testResult.message || 'Invalid credentials or unsupported brand',
        'error',
      )
      submitting.value = false
      return
    ...
    notify('Error', err.message || 'Failed to commit changes to core', 'error')
  } finally {
    submitting.value = false
  }
}
